...
def main():
    ...
    a1_err = 4*simple_histogram_data(50)        # 1D array of error values for a1
    b1_err = 0.1*simple_histogram_data(50)      # 1D array of error values for b1
    ...
    ax[0].spines[['top', 'right']].set_visible(False)       # get rid of top and right border of the plot

    # Here we plot everything on the right pot
    ax[1].set_title('Subplot b')
    ax[1].errorbar(a1, b1, xerr=a1_err, yerr=b1_err, ecolor="#515151", elinewidth=1, barsabove=True)
    # ax[1].legend(loc="upper right")
    ax[1].set_xlabel('x axis [units]')
    ax[1].set_ylabel('y axis [units]')
    ax[1].spines[['top', 'right']].set_visible(False)       # get rid of top and right border of the plot
    plt.show()
